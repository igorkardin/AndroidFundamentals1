...
object KotlinWorkshop3 {
    ...
    private fun printTotalCountOfGuesses(guessCounter: Int) {
        println("Общее количество попыток: $guessCounter")
    }
    ...
    private fun countHigherGuesses(
        ...
        println("Количество попыток попыток воода, которые оказались выше: $counter")
        return counter
    }
    ...
    val countLowerGuesses: (IntArray, Int) -> Unit = { guesses, randomNumber ->
        var count = 0
        for (i in guesses.indices) {
            if (guesses[i] != -1 && guesses[i] < randomNumber) {
                count++
            }
        }
        println("Колисество попыток воода, которые оказались нижe: $count")
    }
